...
flowchart TD
    ...
    subgraph ExternalServices ["External APIs"]
        ...
        BANK["Corporate Bank SLIPS / API"]:::external
    end
    ...
    %% External System Dispatches handled entirely by Backend
    OS -.->|"No Cloud Mode: Direct SMS"| SMS
    CS -.->|"Cloud Mode: Async SMS Event"| SMS
    OS -.->|"Local Mode: Generate Bank File"| BANK
    CS -.->|"Cloud Mode: Secure Bank API"| BANK
```
...
* **Local Office Server (Spring Boot):** Exposes LAN REST endpoints. Leverages Spring Data JPA and `@Transactional` boundaries to ensure all gross-to-net financial calculations are executed with strict ACID compliance. **Handles the secure generation of financial export files to prevent client-side tampering.**
...
* **Server-Side Banking Integration:** To prevent client-side tampering, the Spring Boot backend assumes full responsibility for generating immutable, hash-verified corporate bank files (CSV/XLSX) or transmitting via secure API. The Desktop App merely acts as a trigger and viewer.
